PuchCoin-qt: Qt4 GUI for PuchCoin
===============================
...
Alternatively, install Qt Creator and open the `PuchCoin-qt.pro` file.

An executable named `PuchCoin-qt` will be built.
...
A warning for people using the *static binary* version of PuchCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of PuchCoin is linked against libdb5.3.
...
installed causes PuchCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
