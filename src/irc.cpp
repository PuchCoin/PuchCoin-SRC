...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #PuchCoinTEST2\r");
            Send(hSocket, "WHO #PuchCoinTEST2\r");
        } else {
            // randomly join #PuchCoin00-#PuchCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #PuchCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #PuchCoin%02d\r", channel_number).c_str());
        }
