...
CBigNum bnProofOfStakeLegacyLimit(~uint256(0) >> 17); // 
...
unsigned int nStakeTargetSpacing = 6.6 * 60; 
int64_t nTargetSpacing = 6.6 * 60; 
...
const string strMessageMagic = "PuchCoin Signed Message:\n";
...
static const int64_t nTargetTimespan = 6.6 * 15 * 60;
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
	...
	if (pindexLast->nHeight < 3001)
		return bnTargetLimit.GetCompact(); // low for first 3000 blocks
...
void ThreadScriptCheck(void*) {
    ...
    RenameThread("PuchCoin-scriptch");
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in PuchCoin
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // PuchCoin: check proof-of-stake block signature
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "PuchCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "PUCH GENESIS";
        CTransaction txNew;
        txNew.nTime = 1518554733;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1518554733;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 5519; // !fTestNet ? 1575379 : 46534;
...
unsigned char pchMessageStart[4] = { 0xb6, 0xf8, 0xe1, 0xe2 };
